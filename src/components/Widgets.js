...
export const ProfileCardWidget = (props) => {
  const { image, background, name, position, city } = props
  return (
    <Card border="light" className="text-center p-0 mb-4">
      <div style={{ backgroundImage: `url(${background})` }} className="profile-cover rounded-top" />
      <Card.Body className="pb-5">
        <Card.Img src={image} alt="Neil Portrait" className="user-avatar large-avatar rounded-circle mx-auto mt-n7 mb-4" />
        <Card.Title>{name}</Card.Title>
        <Card.Subtitle className="fw-normal">{position}</Card.Subtitle>
      ...
      <Card.Body>
        <Row className="d-block d-xl-flex align-items-center">
          ...
          <Col xs={12} xl={7} className="px-xl-0">
            ...
            <small>{period}, <FontAwesomeIcon icon={faGlobeEurope} size="xs" /> Okres</small>
